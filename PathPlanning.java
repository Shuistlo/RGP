...
*/
    public class AStar {
        ...
        static int heuristic(Cell current, Cell target) {

            // Q1.Calculate the Manhattan heuristic cost with the input of the two positions
            // You can use Math.abs(a,b) to calculate the absolute value
            int d1 = _______________________________;
            int d2 = _______________________________;
            return d1 + d2;
        }
        //Main function to check the cost of target position and decide whether to put it into
        the close list or open list.
        static void checkAndUpdateCost(Cell current, Cell target, int gcost){

            if(target == null || closed[target.x][target.y])return;

            // Q2.Calculate the total cost for the target position
...
*/
            while(true){
                ...
                // Q3.if current is the target node which means path has been found, return,
                ...
                if(current.x+1<grid.length){
                    ...
                    if(current.y+1<grid[0].length){
                        ...
                        // Q4.Select the right type of cost for this situation
...
// Q5.Fill in the underlined place to perform the AStar function
            ______________; //
            //For visualization
            System.out.println("\nScores for cells: ");
            for(int i=0;i<x;++i){
